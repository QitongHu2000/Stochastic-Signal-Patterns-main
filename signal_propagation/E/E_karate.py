...
# with open('../Networks/karate.gml') as f:
#     gml = f.read()#.decode()
# gml = gml.split("\n")
...
k=1
sources=[33,] #np.random.choice(G_edge.nodes, size=k)
...
save_dict(degree, 'E_karate_degree'+str(int(100*B)))
# save_dict(times_continuous, 'E_karate_continuous'+str(int(100*B)))
# save_dict(times_stochastic, 'E_karate_stochastic'+str(int(100*B)))
save_dict((np.mat(xn)-np.mat(x1))[0,to_nodes], 'E_karate_Deltas_continuous'+str(int(100*B)))
save_dict(times_theory_continuous, 'E_karate_theory_continuous'+str(int(100*B)))
save_dict(times_theory_stochastic, 'E_karate_theory_stochastic'+str(int(100*B)))