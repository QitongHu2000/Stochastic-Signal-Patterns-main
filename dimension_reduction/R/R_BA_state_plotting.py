...
xs_continuous=load_dict('R_BA_xs_continuous'+str(int(100*a))+'_'+str(int(100*b)))
ts_continuous=load_dict('R_BA_ts_continuous'+str(int(100*a))+'_'+str(int(100*b)))[0]
xs_stochastic=load_dict('R_BA_xs_stochastic'+str(int(100*a))+'_'+str(int(100*b)))
ts_stochastic=load_dict('R_BA_ts_stochastic'+str(int(100*a))+'_'+str(int(100*b)))[0]

xs_stochastic=[xs_stochastic[0]-i for i in xs_stochastic]
theory_xs_stochastic=np.power(ts_stochastic[1:],1/2)
theory_xs_stochastic=[np.abs(i/np.mean(theory_xs_stochastic)*np.mean(xs_stochastic)/5) for i in theory_xs_stochastic]
...
plt.savefig('R_BA_state_'+str(int(100*a))+'_'+str(int(100*b))+'.pdf',dpi=300,bbox_inches='tight')
plt.show()