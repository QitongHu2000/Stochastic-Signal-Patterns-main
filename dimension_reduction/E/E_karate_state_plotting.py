...
xs_continuous=load_dict('E_karate_xs_continuous'+str(int(100*B)))
ts_continuous=load_dict('E_karate_ts_continuous'+str(int(100*B)))[0]
xs_stochastic=load_dict('E_karate_xs_stochastic'+str(int(100*B)))
ts_stochastic=load_dict('E_karate_ts_stochastic'+str(int(100*B)))[0]

theory_xs_stochastic=np.power(ts_stochastic[1:],1/2)
theory_xs_stochastic=[np.abs(i/np.mean(theory_xs_stochastic)*np.mean(xs_stochastic)) for i in theory_xs_stochastic]
...
# plt.legend(fontsize=20,loc=1,bbox_to_anchor=(1.1,0.4))
...
plt.savefig('E_karate_state_'+str(int(100*B))+'.pdf',dpi=300,bbox_inches='tight')
plt.show()